...
import java.sql.*;
class RptPerEmpWin implements ActionListener{
	JFrame f1;
...
public RptPerEmpWin(){
	...
	btnshow.setToolTipText("To Show Performance Appraisal Employee ID wise");
...
public void fillempidcombo(){ 
	try{
		...
		rsemp=stmtemp.executeQuery("select * from tblperformance");
		while(rsemp.next()){
...
public void actionPerformed(ActionEvent ae){
	if(ae.getSource()==btnshow){
		...
		try{
			...
			rsuser=stmtselect.executeQuery("select * from tblperformance where empid='"+emp+"'");
			rsuser.last();
			tot=rsuser.getRow();
