...
import java.sql.*;
class RptEmpShiftWin implements ActionListener{
	JFrame f1;
	JLabel lbltitle,lblorgn,lbladd1,lbladd2,lblshift;
	JPanel p0,p1;
	GridLayout glw41;
	Connection conn;
	Statement stmtselect,stmtshift;
	ResultSet rsuser,rsshift;
	Box vb;
	JComboBox cmbshift;
	Object objdata[][];
	JTable jtbldata;
...
public RptEmpShiftWin(){
	...
	btnshow.setToolTipText("To Show Employee Information Shift wise");
	glw41=new GridLayout(4,1);
	cmbshift=new JComboBox(shift1);//JComboBox
	...
	lblshift=new JLabel("Select Shift: ");
	lblshift.setFont(new Font("Franklin Gothic Medium",Font.BOLD,16));
	lblshift.setForeground(new Color(0,0,153));
...
public void actionPerformed(ActionEvent ae){
	if(ae.getSource()==btnshow){
		...
		shift=cmbshift.getSelectedItem().toString();
		try{
			stmtselect=conn.createStatement();
			rsuser=stmtselect.executeQuery("select * from tblemployee where shnm='"+shift+"'");
			rsuser.last();
			tot=rsuser.getRow();
