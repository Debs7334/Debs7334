...
import java.sql.*;
class RptSalMmYyWin implements ActionListener{
	JFrame f1;
	JLabel lbltitle,lblorgn,lbladd1,lbladd2,lblmy;
	JPanel p0,p1;
	GridLayout glw41;
	Connection conn;
	Statement stmtselect,stmtmy;
	ResultSet rsuser,rsmy;
	Box vb;
	JComboBox cmbmy;
	Object objdata[][];
	JTable jtbldata;
...
public RptSalMmYyWin(){
	...
	btnshow.setToolTipText("To Show Salary Slip Month&Year wise");
	glw41=new GridLayout(4,1);
	cmbmy=new JComboBox(my1);//JComboBox
	...
	lblmy=new JLabel("Select Month & Year : ");
	lblmy.setFont(new Font("Franklin Gothic Medium",Font.BOLD,16));
	lblmy.setForeground(new Color(0,0,153));
...
public void fillmycombo(){ 
	try{
		stmtmy=conn.createStatement();
		rsmy=stmtmy.executeQuery("select * from tblsalaryslip");
		while(rsmy.next()){
			my=rsmy.getString("mmyyyy");
			cmbmy.addItem(my);
		}
	}
...
public void actionPerformed(ActionEvent ae){
	if(ae.getSource()==btnshow){
		...
		mmyy=cmbmy.getSelectedItem().toString();
		try{
		stmtselect=conn.createStatement();
		rsuser=stmtselect.executeQuery("select * from tblsalaryslip where mmyyyy='"+mmyy+"'");
		rsuser.last();
		tot=rsuser.getRow();
