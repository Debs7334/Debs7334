...
import java.sql.*;
class RptLeaveShftWin implements ActionListener{
	JFrame f1;
	JLabel lbltitle,lblorgn,lbladd1,lbladd2,lblshft;
	JPanel p0,p1;
	GridLayout glw41;
	Connection conn;
	Statement stmtselect,stmtshft;
	ResultSet rsuser,rsshft;
	Box vb;
	JComboBox cmbshft;
	Object objdata[][];
	JTable jtbldata;
	...
	String shft,shnm;
	String shft1[]={"-Select-"};
	String colhead[]={"Serial No.","Reference No.","Date","Employee ID","Employee Name","Gender","Department Name","Shift Name","Start Time","End Time","Leave Type","Date From","Date To","Narration"};
	int slno,rw,tot;
	JButton btnclose,btnshow;
	ImageIcon imgclose;
	JOptionPane msg;
public RptLeaveShftWin(){
	f1=new JFrame();
	p0=new JPanel();
	p1=new JPanel();
	...
	btnshow.setToolTipText("To Show Leave Application Shift wise");
	glw41=new GridLayout(4,1);
	cmbshft=new JComboBox(shft1);//JComboBox
	...
	lblshft=new JLabel("Select Shift: ");
	lblshft.setFont(new Font("Franklin Gothic Medium",Font.BOLD,16));
	lblshft.setForeground(new Color(0,0,153));
...
public void fillshftcombo(){ 
	try{
		stmtshft=conn.createStatement();
		rsshft=stmtshft.executeQuery("select * from tblshift");
		while(rsshft.next()){
			shnm=rsshft.getString("sfnm");
			cmbshft.addItem(shnm);
		}
	}
...
public void actionPerformed(ActionEvent ae){
	if(ae.getSource()==btnshow){
		...
		shft=cmbshft.getSelectedItem().toString();
		try{
			stmtselect=conn.createStatement();
			rsuser=stmtselect.executeQuery("select * from tblleave where shnm='"+shft+"'");
			rsuser.last();
			tot=rsuser.getRow();
			objdata=new Object[tot][14];
			slno=1;
			rw=0;
			rsuser.beforeFirst();
			while(rsuser.next()){
				objdata[rw][0]=slno;
				objdata[rw][1]=rsuser.getString("refno");
				...
				objdata[rw][10]=rsuser.getString("ltype");
				objdata[rw][11]=rsuser.getString("dfrom");
				objdata[rw][12]=rsuser.getString("dto");
				objdata[rw][13]=rsuser.getString("nar");
				slno=slno+1;
				rw=rw+1;
			}
		}
