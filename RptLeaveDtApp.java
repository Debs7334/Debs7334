...
import java.sql.*;
class RptLeaveDtWin implements ActionListener{
	JFrame f1;
...
public RptLeaveDtWin(){
	...
	btnshow.setToolTipText("To Show Leave Application Date wise");
...
public void filldtcombo(){ 
	try{
		...
		rsdt=stmtdt.executeQuery("select * from tblleave");
		while(rsdt.next()){
...
public void actionPerformed(ActionEvent ae){
	if(ae.getSource()==btnshow){
		...
		try{
			...
			rsuser=stmtselect.executeQuery("select * from tblleave where daten='"+date+"'");
			rsuser.last();
			tot=rsuser.getRow();
