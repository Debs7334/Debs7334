...
import java.sql.*;
class RptEmpGenWin implements ActionListener{
	JFrame f1;
	JLabel lbltitle,lblorgn,lbladd1,lbladd2,lblgen;
	JPanel p0,p1;
	GridLayout glw41;
	Connection conn;
	Statement stmtselect,stmtgen;
	ResultSet rsuser,rsgen;
	Box vb;
	JComboBox cmbgen;
	Object objdata[][];
	JTable jtbldata;
	JScrollPane jspdata;
	String gen;
	String gen1[]={"-Select-","Male","Female"};
...
public RptEmpGenWin(){
	...
	btnshow.setToolTipText("To Show Employee Information Gender wise");
	glw41=new GridLayout(4,1);
	cmbgen=new JComboBox(gen1);//JComboBox
	...
	lblgen.setFont(new Font("Franklin Gothic Medium",Font.BOLD,16));
	lblgen.setForeground(new Color(0,0,153));
...
public void actionPerformed(ActionEvent ae){
	if(ae.getSource()==btnshow){
		...
		try{
			...
			rsuser=stmtselect.executeQuery("select * from tblemployee where gender='"+gen+"'");
			rsuser.last();
			tot=rsuser.getRow();
