...
import java.sql.*;
class RptPerDeptWin implements ActionListener{
	JFrame f1;
...
public RptPerDeptWin(){
	...
	btnshow.setToolTipText("To Show Performance Appraisal Department wise");
...
public void actionPerformed(ActionEvent ae){
	if(ae.getSource()==btnshow){
		...
		try{
			...
			rsuser=stmtselect.executeQuery("select * from tblperformance where dpnm='"+dept+"'");
			rsuser.last();
			tot=rsuser.getRow();
