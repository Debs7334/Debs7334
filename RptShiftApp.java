...
import java.sql.*;
class RptShiftWin implements ActionListener{
	JFrame f1;
	...
	JTable jtbldata;
	JScrollPane jspdata;
	String colhead[]={"Serial No.","Shift Code","Shift Name","Start Time","End Time"};
	int slno,rw,tot;
	JButton btnclose;
	ImageIcon imgclose;
	JOptionPane msg;
public RptShiftWin(){
	...
	lbltitle=new JLabel("SHIFT  INFORMATION",JLabel.CENTER);
...
public void doconnect(){
	...
	try{
		...
		while(rsuser.next()){
			objdata[rw][0]=slno;
			objdata[rw][1]=rsuser.getString("sfcd");
			objdata[rw][2]=rsuser.getString("sfnm");
			objdata[rw][3]=rsuser.getString("srttym");
			objdata[rw][4]=rsuser.getString("endtym");
			slno=slno+1;
			rw=rw+1;
		}
	}//try ends
