...
import java.sql.*;
class RptLeaveEmpWin implements ActionListener{
	JFrame f1;
...
public RptLeaveEmpWin(){
	...
	btnshow.setToolTipText("To Show Leave Application Employee ID wise");
...
public void fillempidcombo(){ 
	try{
		...
		rsemp=stmtemp.executeQuery("select * from tblleave");
		while(rsemp.next()){
...
public void actionPerformed(ActionEvent ae){
	if(ae.getSource()==btnshow){
		...
		try{
			...
			rsuser=stmtselect.executeQuery("select * from tblleave where empid='"+emp+"'");
			rsuser.last();
			tot=rsuser.getRow();
