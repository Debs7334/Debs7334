...
import java.sql.*;
class RptAprDeptWin implements ActionListener{
	JFrame f1;
...
public RptAprDeptWin(){
	...
	btnshow.setToolTipText("To Show Leave Approval Department wise");
...
public void actionPerformed(ActionEvent ae){
	if(ae.getSource()==btnshow){
		...
		try{
			...
			rsuser=stmtselect.executeQuery("select * from tblapproval where dpnm='"+dept+"'");
			rsuser.last();
			tot=rsuser.getRow();
