...
import java.sql.*;
class RptAprEmpWin implements ActionListener{
	JFrame f1;
...
public RptAprEmpWin(){
	...
	btnshow.setToolTipText("To Show Leave Approval Employee ID wise");
...
public void fillempidcombo(){ 
	try{
		...
		rsemp=stmtemp.executeQuery("select * from tblapproval");
		while(rsemp.next()){
...
public void actionPerformed(ActionEvent ae){
	if(ae.getSource()==btnshow){
		...
		try{
			...
			rsuser=stmtselect.executeQuery("select * from tblapproval where empid='"+emp+"'");
			rsuser.last();
			tot=rsuser.getRow();
