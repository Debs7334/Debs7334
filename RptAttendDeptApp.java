...
import java.sql.*;
class RptAttendDeptWin implements ActionListener{
	JFrame f1;
...
public RptAttendDeptWin(){
	...
	btnshow.setToolTipText("To Show Attendance Register Department wise");
...
public void actionPerformed(ActionEvent ae){
	if(ae.getSource()==btnshow){
		...
		try{
			...
			rsuser=stmtselect.executeQuery("select * from tblattendance where dpnm='"+dept+"'");
			rsuser.last();
			tot=rsuser.getRow();
