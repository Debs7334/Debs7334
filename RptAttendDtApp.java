...
import java.sql.*;
class RptAttendDtWin implements ActionListener{
	JFrame f1;
...
public RptAttendDtWin(){
	...
	btnshow.setToolTipText("To Show Attendance Register Date wise");
...
public void filldtcombo(){ 
	try{
		...
		rsdt=stmtdt.executeQuery("select * from tblattendance");
		while(rsdt.next()){
...
public void actionPerformed(ActionEvent ae){
	if(ae.getSource()==btnshow){
		...
		try{
			...
			rsuser=stmtselect.executeQuery("select * from tblattendance where daten='"+date+"'");
			rsuser.last();
			tot=rsuser.getRow();
